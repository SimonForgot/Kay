...
void Rtcore::addGeo(ptr<float> vex, ptr<unsigned int> ind, int vcnt, int fcnt) {
  ...
  rtcSetSharedGeometryBuffer(geom, RTC_BUFFER_TYPE_VERTEX, 0, RTC_FORMAT_FLOAT3,
                             vertices, 0, 3 * sizeof(float), vcnt);
  rtcSetSharedGeometryBuffer(geom, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3,
                             indices, 0, 3 * sizeof(unsigned), fcnt);
  rtcCommitGeometry(geom);
  rtcAttachGeometry(scene, geom);
  rtcReleaseGeometry(geom);
}

Rtrc Rtcore::intersect(float ox, float oy, float oz, float dx, float dy,
                       float dz) {
...
PYBIND11_MODULE(kay, m) {
  ...
  py::class_<ptr<float>>(m, "float_ptr").def(py::init<std::size_t>());
